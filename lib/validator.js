...
function isObject() {
  ...
  function validate(value, onError) {
    ...
    if (typeof value !== 'object') {
      return onError('Incorrect type. Expects object.');
    }

    var result = null;
    // check for unExpects values
    for (var parameter in value) {
      if (rules[parameter]) {
        continue;
      }

      result |= onError('UnExpects value.', parameter, value[parameter]);
    }
...
function isString(options) {
  ...
  function validate(value, onError) {
    ...
    if (!util.isString(value)) {
      return onError('Incorrect type. Expects string.');
    }
    if (options) {
...
function isBoolean() {
  ...
  function validate(value, onError) {
    ...
    if (!util.isBoolean(value)) {
      return onError('Incorrect type. Expects boolean.');
    }
    return null;
  }
}
...
function isNumber(options) {
  ...
  function validate(value, onError) {
    ...
    if (!acceptType || isNaN(value)) {
      return onError('Incorrect type. Expects number.');
    }
    if (options) {
  ...
  function validate(value, onError) {
    ...
    if (options.format) {
      if (!moment(value, options.format, true).isValid()) {
        return onError(
          options.message ||
            'Invalid value. Format is not valid. Expects ' + options.format,
        );
      }
    } else {
...
function isArray(validator, options) {
  ...
  function validate(value, onError) {
    ...
    if (!(value instanceof Array)) {
      return onError('Incorrect type. Expects array.');
    }
    if (options) {
